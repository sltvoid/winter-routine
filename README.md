...
| [`email-calendar-scan.md`](email-calendar-scan.md) | Daily 05:40 ET (Code routine "Email Calendar Scan", `CRON_TZ=America/Toronto 40 5 * * *`, repo attached, Google-Calendar connector; paste body `claude-routine-email-calendar-scan.v1.md`) | Creates/updates future events on **Steph Main** only from MCP email candidates (`prepare_email_calendar_scan`, 48 h window); every decision recorded via `record_email_calendar_decision`; never `primary`/`CC`; no Gmail connector use; Proton mail included since v3 (2026-09-26, origin `proton`) |
...
| [`program-review.md`](program-review.md) | Sunday 21:15 ET lifeOS week composer (trigger "Weekly Program Review", `CRON_TZ=America/Toronto 15 21 * * 0`; after the verifier's 20:35 rollup, the 05:37 insight sweep and the 09:07 scorecard) | 1 `program_versions` row via `write_program` (content auto-activates; frame deltas clamp to draft) + review notes in `agent_runs` incl. the weekly **platform-governance** section (absorbs the retired Gemini daily reviews, 2026-07-02) — now also lists research verdicts (recommendation + finding) for `research_complete` tickets |
| [`learning-agent.md`](learning-agent.md) | Monthly learner — first Sunday ~12:00 ET (trigger fires every Sunday noon, the body gate skips the rest) + commissioned on phase changes; evidence packet `scripts/learner_evidence.py` | `llm_runs` + `agent_runs`; profile/memory writes only when fresh evidence passes replay guard and audit |
